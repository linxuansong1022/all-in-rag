...
"""
=== Rerank (重排序) 与 Refine (精炼) - 优化版 ===

【文件说明】：
这个脚本与 `07_rerank_and_refine.py` 的逻辑基本一致，但做了针对性的优化：
1. **中文分块策略更细致**：使用了更适合中文的标点符号作为分隔符，并且分块更小（300字）。
2. **应用场景优化**：更小的分块意味着 ColBERT 的计算粒度更细，有助于提升检索的精准度。

【核心思想】：
通过 ColBERT 精排 + LLM 精炼，从大量文档中“沙里淘金”，提取最精准的上下文给大模型。
"""
...
# ==========================================
# 3. 准备数据 - 优化版 (Optimized Data Prep)
# ==========================================
# 加载示例文本
print("正在加载并切分文档...")
...
# 【优化点】：优化分块策略
# 1. 明确指定中文标点符号作为分隔符，防止句子被切断。
# 2. chunk_size=300: 相比之前的 500，这里使用了更小的块。
#    更小的块意味着 ColBERT 每个块通过 BERT 计算的 token 更少，但也意味着总块数变多了。
#    这有利于定位更精确的段落。
...
# 4.4 组装管道 (Pipeline)
# 流程: ColBERT重排 (20 -> 5) -> LLM 精炼 (提取关键句)
